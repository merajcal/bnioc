  ...
  useEffect(() => {
    ...
    getMyRegistration(match.id, user.token)
      .then((result) => {
        ...
        const paymentRequired = registration?.status === 'payment_pending' && !registration.paymentStatus;
        if (!cancelled) setExistingRegistration(paymentRequired ? null : registration);
      })
...
const registrationStatusLabel = (status) => ({ payment_pending: 'Payment required', confirmed: 'Player confirmed', rejected: 'Player rejected' }[status] || status);
...
function PlayerActionsMenu({ registration, onAcknowledgePayment, onChangeStatus, onPlayerRole, onRemovePlayer }) {
  ...
  return <div className="relative shrink-0">
    ...
    {open && <div className="absolute right-0 top-11 z-20 w-52 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
      ...
      {registration.status !== 'confirmed' && registration.paymentStatus === 'verified' && <button onClick={() => action(() => onChangeStatus(registration.id, 'confirmed'))} className="block w-full rounded-xl px-3 py-2 text-left text-sm font-semibold text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/30">Confirm player</button>}
...
function MatchWorkspace({ match, registrations, activeTab, setActiveTab, playerForm, setPlayerForm, onSubmitPlayer, onToggleMatch, onCancelMatch, onEditMatch, onAcknowledgePayment, onChangeStatus, onPlayerRole, onRemovePlayer }) {
  ...
  return <section className="space-y-5">
    ...
    {activeTab === 'players' ? <>
      {canManageRoster && <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-100 p-5 dark:border-slate-700 dark:bg-slate-800"><div className="mb-4"><h3 className="text-lg font-black text-slate-900 dark:text-white">Add player to {match.title}</h3><p className="mt-1 text-sm text-slate-700 dark:text-slate-300">The player will be reserved as payment pending. They must sign in with the same email or phone and pay the match fee before becoming confirmed.</p></div><form onSubmit={onSubmitPlayer} className="grid gap-3 sm:grid-cols-[1fr_1fr_1fr_auto]"><input required className={inputClass} value={playerForm.playerName} onChange={(e) => setPlayerForm({ ...playerForm, playerName: e.target.value })} placeholder="Full player name" /><input type="email" className={inputClass} value={playerForm.email} onChange={(e) => setPlayerForm({ ...playerForm, email: e.target.value })} placeholder="Email (recommended)" /><input type="tel" className={inputClass} value={playerForm.phone} onChange={(e) => setPlayerForm({ ...playerForm, phone: e.target.value })} placeholder="Phone (required if no email)" /><button className="rounded-xl bg-primary-500 px-4 py-3 text-sm font-bold text-white hover:bg-primary-600">Add for payment</button></form></div>}

  ...
  return <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><button onClick={onBack} className="mb-5 inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600 transition hover:border-primary-500 hover:text-primary-600 dark:border-slate-700 dark:text-slate-300">← Back to matches</button>
    ...
    <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="space-y-6">
        <div className="grid grid-cols-3 gap-3"><div className="rounded-2xl bg-slate-900 p-4 text-white"><p className="text-xs text-slate-400">Active</p><p className="mt-1 text-2xl font-black">{data.matches.filter((item) => item.status === 'active').length}</p></div><div className="rounded-2xl bg-orange-500 p-4 text-white"><p className="text-xs text-orange-100">Players</p><p className="mt-1 text-2xl font-black">{data.registrations.length}</p></div><div className="rounded-2xl bg-emerald-600 p-4 text-white"><p className="text-xs text-emerald-100">Confirmed</p><p className="mt-1 text-2xl font-black">{data.registrations.filter((item) => item.status === 'confirmed').length}</p></div></div>

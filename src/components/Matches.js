...
import { getAdminData, getMatches, login, registerStudent, createMatch, submitRegistration, updateRegistration, updateMatchStatus, addPlayer, removePlayer } from '../services/matchApi';
...
export function AuthPanel({ onSuccess, onClose }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" onClick={onClose}>
    <div className="w-full max-w-md rounded-3xl bg-white p-7 shadow-2xl dark:bg-slate-900" onClick={(event) => event.stopPropagation()}>
      ...
      {mode === 'login' && <p className="mb-5 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">Your account type is detected automatically after sign in.</p>}
...
const paymentStatusLabel = (status) => ({ submitted: 'Pending acknowledgement', verified: 'Payment completed', rejected: 'Payment rejected' }[status] || 'No payment');
...
function AdminDashboard({ user, matches, onCreated, onMatchUpdated, onRosterChanged = () => {}, onLogout, onBack = () => window.location.assign('/matches') }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setForm(emptyMatch); setNotice('Match saved as inactive. Make it active when registration should open.'); onCreated(created); setData((old) => ({ ...old, matches: [created, ...old.matches] }));
  ...
  const acknowledgePayment = async (registration, status) => {
    ...
    try {
      ...
      onRosterChanged();
      setNotice(status === 'verified' ? `${registration.playerName}'s payment is acknowledged and marked completed.` : `${registration.playerName}'s payment was rejected.`);
  ...
  return <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><button onClick={onBack} className="mb-5 inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600 transition hover:border-primary-500 hover:text-primary-600 dark:border-slate-700 dark:text-slate-300">← Back to matches</button>
    ...
    <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"><h3 className="text-xl font-black text-slate-900 dark:text-white">Publish a match</h3><p className="mb-6 mt-1 text-sm text-slate-500">The form automatically selects the jersey guidance from ball type.</p><form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Match title"><input required className={inputClass} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. BNIOC U14 Match Day" /></Field><Field label="Opponent"><input required className={inputClass} value={form.opponent} onChange={(e) => setForm({ ...form, opponent: e.target.value })} placeholder="e.g. Whitefield Cricket Club" /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Match type"><select className={inputClass} value={form.matchType} onChange={(e) => setForm({ ...form, matchType: e.target.value })}><option>U14</option><option>U16</option><option>U19</option><option>Tournament</option><option>Friendly</option><option>Other</option></select></Field><Field label="Match date"><input required type="date" min={new Date().toISOString().slice(0, 10)} className={inputClass} value={form.matchDate} onChange={(e) => setForm({ ...form, matchDate: e.target.value })} /></Field><Field label="Match fee (INR)"><input required min="0" type="number" className={inputClass} value={form.matchFee} onChange={(e) => setForm({ ...form, matchFee: e.target.value })} placeholder="750" /></Field><Field label="Reporting time"><input required type="time" className={inputClass} value={form.reportingTime} onChange={(e) => setForm({ ...form, reportingTime: e.target.value })} /></Field></div><Field label="Location"><input required className={inputClass} value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="BNIOC Ittangur Cricket Ground" /></Field><Field label="Google Maps link"><input required type="url" className={inputClass} value={form.mapsUrl} onChange={(e) => setForm({ ...form, mapsUrl: e.target.value })} placeholder="https://maps.google.com/..." /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Ball type"><div className="grid grid-cols-2 gap-2">{['white', 'red'].map((ball) => <button type="button" key={ball} onClick={() => setForm({ ...form, ballType: ball })} className={`rounded-xl border px-3 py-3 text-sm font-bold capitalize ${form.ballType === ball ? 'border-primary-500 bg-primary-50 text-primary-700 dark:bg-primary-950/40 dark:text-primary-300' : 'border-slate-200 text-slate-500 dark:border-slate-700'}`}>{ball} ball</button>)}</div></Field><div className="flex items-center gap-2 pt-6"><span className="h-2.5 w-2.5 shrink-0 rounded-full bg-primary-500" aria-hidden="true" /><p className="text-sm font-semibold text-slate-300">{form.ballType === 'red' ? 'White Jersey' : 'Color Jersey'}</p></div></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Player capacity"><input required min="1" type="number" className={inputClass} value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })} /></Field><Field label="Overs"><input required min="1" step="1" type="number" className={inputClass} value={form.overs} onChange={(e) => setForm({ ...form, overs: e.target.value })} /></Field></div><button className="w-full rounded-xl bg-primary-500 px-4 py-3.5 font-bold text-white hover:bg-primary-600">Publish match</button></form></section>
      <section className="space-y-6">
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-4"><div className="rounded-2xl bg-slate-900 p-4 text-white"><p className="text-xs text-slate-400">Active matches</p><p className="mt-1 text-2xl font-black">{data.matches.filter((item) => item.status === 'active').length}</p></div><div className="rounded-2xl bg-amber-500 p-4 text-white"><p className="text-xs text-amber-100">Pending payments</p><p className="mt-1 text-2xl font-black">{data.registrations.filter((item) => item.paymentStatus === 'submitted').length}</p></div><div className="rounded-2xl bg-emerald-600 p-4 text-white"><p className="text-xs text-emerald-100">Payments completed</p><p className="mt-1 text-2xl font-black">{data.registrations.filter((item) => item.paymentStatus === 'verified').length}</p></div><div className="rounded-2xl bg-orange-500 p-4 text-white"><p className="text-xs text-orange-100">Confirmed players</p><p className="mt-1 text-2xl font-black">{data.registrations.filter((item) => item.status === 'confirmed').length}</p></div></div>
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"><h3 className="mb-4 text-xl font-black text-slate-900 dark:text-white">All matches</h3><div className="space-y-3">{data.matches.map((match) => <div key={match.id} className="rounded-2xl border border-slate-100 p-4 dark:border-slate-800"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-slate-900 dark:text-white">{match.title}</p><p className="mt-1 text-xs text-slate-500">{formatDate(match.matchDate)} · {match.overs} overs · {match.registrationsCount || 0}/{match.capacity} players</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${match.status === 'cancelled' ? 'bg-red-50 text-red-700' : match.status === 'inactive' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>{match.status}</span></div>{match.status !== 'cancelled' && <div className="mt-3 flex flex-wrap items-center gap-3"><MatchShareActions match={match} /><button onClick={() => handleToggleMatch(match)} className="rounded-lg border border-primary-200 px-3 py-1.5 text-xs font-bold text-primary-700 dark:border-primary-800 dark:text-primary-300">{match.status === 'active' ? 'Set inactive' : 'Make active'}</button><button onClick={() => handleCancelMatch(match)} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-bold text-red-600">Cancel match</button></div>}</div>)}</div></div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"><h3 className="mb-4 text-xl font-black text-slate-900 dark:text-white">Players & payment proofs</h3>{data.registrations.length === 0 ? <p className="text-sm text-slate-500">Registrations and manually added players will appear here.</p> : <div className="space-y-3">{data.registrations.map((registration) => <div key={registration.id} className="rounded-2xl border border-slate-100 p-4 dark:border-slate-800"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-bold text-slate-900 dark:text-white">{registration.playerName}</p><p className="text-xs text-slate-500">{matchById[registration.matchId]?.title || 'Match'} · {registration.email || 'No email'}{registration.paymentTransactionId ? ` · ${registration.paymentTransactionId}` : ''}</p></div><div className="flex flex-wrap gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${registration.status === 'confirmed' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{registration.status.replace('_', ' ')}</span>{registration.paymentStatus && <span className={`${paymentStatusClass(registration.paymentStatus)} rounded-full px-2 py-1 text-[10px] font-black uppercase`}>{paymentStatusLabel(registration.paymentStatus)}</span>}</div></div><div className="mt-3 flex flex-wrap gap-2">{registration.paymentStatus === 'submitted' && <><button onClick={() => acknowledgePayment(registration, 'verified')} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white">Acknowledge payment</button><button onClick={() => acknowledgePayment(registration, 'rejected')} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-bold text-red-600">Reject payment</button></>}{registration.status !== 'confirmed' && <button onClick={() => changeStatus(registration.id, 'confirmed')} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white">Confirm</button>}{registration.status !== 'rejected' && <button onClick={() => changeStatus(registration.id, 'rejected')} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-bold text-red-600">Reject</button>}{registration.status === 'confirmed' && <><button onClick={() => handlePlayerRole(registration, 'captain')} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${registration.isCaptain ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300'}`}>{registration.isCaptain ? 'Captain ✓' : 'Make captain'}</button><button onClick={() => handlePlayerRole(registration, 'wicket keeper')} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${registration.isWicketKeeper ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300'}`}>{registration.isWicketKeeper ? 'Wicket keeper ✓' : 'Make wicket keeper'}</button></>}{registration.status !== 'rejected' && <button onClick={() => handleRemovePlayer(registration)} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 dark:border-slate-700 dark:text-slate-300">Remove</button>}</div></div>)}</div>}</div>
      </section>
    </div>
  </div>;
}
